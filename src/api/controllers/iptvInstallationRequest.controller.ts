...
export const addIptvInstallationRequest = async (req: Request, res: Response): Promise<any> => {
  try {
    const { 
      name, 
      email, 
    ...
    const existingRequest = await IptvInstallationRequest.findOne({
      userId,
    ...
    if (existingRequest) {
      return res.status(400).json({
        success: false,
        message: 'User already has a pending or approved IPTV installation request'
      });
    }

    const newRequest = new IptvInstallationRequest({
      userId,
      name,
      email,
      phoneNumber,
    ...
    res.status(201).json({
      success: true,
      message: 'IPTV installation request created successfully',
      data: savedRequest
    });
  } catch (error) {
    console.error('Error creating IPTV installation request:', error);
    res.status(500).json({
      success: false,
...
// Get all IPTV installation requests
export const getAllIptvInstallationRequests = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const requests = await IptvInstallationRequest.find(filter)
    ...
    const total = await IptvInstallationRequest.countDocuments(filter);

    res.status(200).json({
      success: true,
      message: 'IPTV installation requests retrieved successfully',
      data: {
        requests,
        pagination: {
    ...
    console.error('Error fetching IPTV installation requests:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error instanceof Error ? error.message : 'Unknown error'
...
export const getInstallRequestById = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const request = await IptvInstallationRequest.findById(id)
      .populate('userId', 'name email phoneNumber countryCode')
      .populate('assignedEngineer', 'name email phoneNumber')
      .populate('iptvPlanId', 'name price description features');

    if (!request) {
      return res.status(404).json({
        success: false,
        message: 'IPTV installation request not found'
      });
    }

    res.status(200).json({
      success: true,
      message: 'IPTV installation request retrieved successfully',
      data: request
    });
  } catch (error) {
    console.error('Error fetching IPTV installation request:', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error instanceof Error ? error.message : 'Unknown error'
...
// Assign engineer to IPTV installation request
export const assignEngineer = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const request = await IptvInstallationRequest.findByIdAndUpdate(
      id,
...
export const declineRequest = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const updatedRequest = await IptvInstallationRequest.findByIdAndUpdate(
      id,
      {
        status: 'rejected',
...
export const getRequestsByUserId = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: 'User IPTV installation requests retrieved successfully',
      data: requests
    });
  } catch (error) {
